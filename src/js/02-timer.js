...
function onClickTimer() {
    Notify.success('Start', {opacity: 0.8, position: "center-top", timeout: 300, cssAnimationDuration: 800, backOverlay: true, backOverlayColor: "rgba(50,198,130,0.2)", cssAnimationStyle: "zoom" }); 
      ...
      const setId = setInterval(() => {
        dateDiff = (selectedDates[0].getTime()) - (new Date().getTime());
            if(dateDiff <= 0 ){
                clearInterval(setId);
                refs.startBtnRef.disabled = true;
                Notify.failure('Please choose a date in the future', {opacity: 0.8, position: "center-top", timeout: 300, backOverlay: true, cssAnimationDuration: 800, backOverlayColor: "rgba(255,85,73,0.2)", cssAnimationStyle: "zoom" });
                return   
            }
        }, 1000)
